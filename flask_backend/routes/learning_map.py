...
from services.llm import generate_learning_map_template
...
def generate_map():
    ...
    if sp:
        try:
            current_skills = list(json.loads(sp.skills_json).keys())
        except Exception:
            current_skills = []
    ...
    if resume_match_id:
        ...
        if rm and rm.missing_skills:
            try:
                missing = json.loads(rm.missing_skills)
            except Exception:
                missing = []

    out = generate_learning_map_template(target_role, missing, current_skills)
...
def latest_map():
    user_id = int(get_jwt_identity())
    lm = (LearningMap.query.filter_by(user_id=user_id)
          .order_by(LearningMap.created_at.desc()).first())
    if not lm:
        return jsonify({"learning_map": None})
    return jsonify({
        "learning_map": {
            "id": lm.id,
            "target_role": lm.target_role,
            "created_at": lm.created_at.isoformat(),
            "roadmap": json.loads(lm.roadmap_json),
            "markdown": lm.roadmap_markdown
        }
    })
